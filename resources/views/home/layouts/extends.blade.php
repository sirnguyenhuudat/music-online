...
@section ('script')
    <script>
        ...
        @php
            if (isset($_COOKIE['trackCurrent'])) {
                $trackCurrent = json_decode($_COOKIE['trackCurrent'], true);
        @endphp
        $('.audio-player').show();
        var trackCurrent = {
            image : '{{ asset($trackCurrent['picture']) }}',
            title : '{{ $trackCurrent['name'] }}',
            artist : '{{ $trackCurrent['artist'] }}',
            mp3 : '{{ $trackCurrent['path'] }}',
